...
#############
# https://github.com/moikot/golang-dep/blob/8234ce5a98f109d08fdc3ec2fac9a8e22b433a11/.travis/push.sh
#############
#
#
...
# Examples:
#
#   push_readme "token" "foo/bar" "content"
#
push_readme() {
  declare -r token="${1}"
  declare -r image="${2}"
  ...
  local code=$(jq -n --arg msg "${content}" \
    '{"full_description": $msg }' | \
...
# Examples:
#
#   get_token "dockerhubuser" "dockerhubpassword"
#
get_token() {
...
push() {
  ...
  local token=$(get_token $DOCKER_USERNAME $DOCKER_PASSWORD)
